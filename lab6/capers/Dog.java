...
public class Dog implements Serializable{ // TODO
    ...
    public static Dog fromFile(String name) {
        File aimFile = Utils.join(DOG_FOLDER,name);
        if(!aimFile.exists()){
            throw new NoSuchElementException("No such dog");
        }
        else{
            try{
                return Utils.readObject(aimFile,Dog.class);
            } catch (Exception e){
                Utils.exitWithError("read dog failure.");
            }
        }
        return null;
    }
    ...
    public void saveDog() {
        ...
        if(!dogFile.exists()){
            try{
                ...
                Utils.exitWithError("create dogFile failure");
            }
        }
